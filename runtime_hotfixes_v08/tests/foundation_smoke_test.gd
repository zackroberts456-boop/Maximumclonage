...
func _initialize():
    ...
    if failures.is_empty():
        print("MAXIMUM CLONAGE V0.8 SMOKE TEST: PASS")
        quit(0)
    else:
        for failure in failures:
            push_error(failure)
        print("MAXIMUM CLONAGE V0.8 SMOKE TEST: FAIL (%d)" % failures.size())
        quit(1)
